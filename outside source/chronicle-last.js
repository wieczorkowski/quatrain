...
const CHRONICLE_VERSION = '0.1.4' // Last revision for strategy management
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (msg) => {
        ...
        if (action === 'set_client_id') {
        ...
        } else if (action === 'get_anno') {
            // Handle annotation retrieval
            const { clientid, instrument, timeframe } = request;
        ...
        } else if (action === 'save_anno') {
            // Handle annotation saving (create or update)
                ...
                function(err) {
                    if (err) {
                        ...
                        db.get('SELECT client_id FROM STRATEGIES WHERE client_id = ?', [clientid], (err, row) => {
                            ...
                            if (row) {
                                // This annotation is from a registered strategy - broadcast to all clients
                                console.log(`Broadcasting strategy annotation update for client ${clientid}`);
                                wss.clients.forEach(client => {
                                    if (client.readyState === WebSocket.OPEN) {
                                        client.send(JSON.stringify({
                                            mtyp: 'ctrl',
                                            action: 'strategy-anno-update',
                                            clientid: clientid
                                        }));
                                    }
                                });
            ...
            if (clientid && clientid !== 'all') {
                db.get('SELECT client_id FROM STRATEGIES WHERE client_id = ?', [clientid], (err, stratRow) => {
                    if (err) {
                        console.error('Error checking if annotation is from a registered strategy:', err.message);
                    }
                    
                    // Now execute the delete
                    executeDelete(stratRow ? true : false);
                });
            } else {
            ...
            function executeDelete(isStrategy) {
                ...
                db.run(query, params, function(err) {
                    if (err) {
                        ...
                        // If this was a strategy annotation, broadcast to all clients
                        if (isStrategy && this.changes > 0) {
                            console.log(`Broadcasting strategy annotation deletion for client ${clientid}`);
                            wss.clients.forEach(client => {
                                if (client.readyState === WebSocket.OPEN) {
                                    client.send(JSON.stringify({
                                        mtyp: 'ctrl',
            ...
            if (reinit) {
                ...
                db.run('DELETE FROM ANNOTATIONS WHERE client_id = ?', [clientid], function(err) {
                    ...
                    db.run(
                        'INSERT OR REPLACE INTO STRATEGIES (client_id, strategy_name, description, parameters) VALUES (?, ?, ?, ?)',
                        [clientid, name, description, parametersJson],
                        function(err) {
                            if (err) {
                ...
                db.get('SELECT client_id FROM STRATEGIES WHERE client_id = ?', [clientid], (err, row) => {
                    ...
                    if (row) {
                        ...
                        // Insert new strategy
                        db.run(
                            'INSERT INTO STRATEGIES (client_id, strategy_name, description, parameters) VALUES (?, ?, ?, ?)',
                            [clientid, name, description, parametersJson],
                            function(err) {
                                if (err) {
                                    console.error('Error registering strategy:', err.message);
        ...
        } else if (action === 'unregister_strat') {
            // Handle strategy unregistration
            const { clientid } = request;
        ...
        } else if (action === 'get_strat') {
            // Handle strategy retrieval
            ...
            // Build the query
            let query = 'SELECT client_id, strategy_name, description, parameters FROM STRATEGIES';
            if (whereClause.length > 0) {
                query += ' WHERE ' + whereClause.join(' AND ');
            }
